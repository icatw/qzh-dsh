...
In addition to the three-column layout and `ctx.layout` panel service, this package provides the `ctx.workbench` product-workbench selector. It defaults to `qzh` and is shared by the front-line switcher and session browser.
