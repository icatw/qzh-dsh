...
The sidebar has a fixed `Log analysis` / `General chat` session-type switch under the brand. Switching changes the visible and newly-created session target without duplicating DSH's session framework.
