...
- **普通工具详情仍没有通用入口**：对于已选择的工具调用，`ChatViewInjected.openDetails` 虽已实现却无人调用，因此组装后的应用仍无法主动打开原始工具详情。QZH 会话是例外：证据面板通过 `conversation.details.qzh` 注册，并由 QZH 会话操作打开。没有 Input/Output/Metadata 切换、Prev/Next 步进，也没有 trajectory 深链接。
