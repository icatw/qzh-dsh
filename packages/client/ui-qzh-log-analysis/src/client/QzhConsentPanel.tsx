    ...
    <section className={css.consentCard} aria-labelledby="qzh-consent-heading">
      <div className={css.sectionHeading}>
        <div>
          ...
          <p>只发送上方「确认发送内容」中列出的文件清单、样例和完整日志包，提交后由 QZH Agent 读取只读源码镜像。</p>
        </div>
      ...
      <div className={css.formGrid}>
        ...
        <label className={css.formWide}>
          现场现象（可选）
          <textarea value={failureDescription} onChange={(event) => { onFailureDescription(event.currentTarget.value) }} rows={3} />
        </label>
      </div>
      <label className={css.consentLabel}>
        <input type="checkbox" checked={consent} onChange={(event) => { onConsent(event.currentTarget.checked) }} />
        <span>我确认将上方预览的文件清单、错误样例和完整日志包发送到公司内网 QZH 分析 Host，用于故障分析。</span>
      </label>
    </section>
  )
}
