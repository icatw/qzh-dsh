...
浏览器按 `ctx.workbench` 过滤会话：QZH 工作台只显示 `agentPreset: qzh`，通用 DSH 工作台显示其他会话；新建会话时会把对应 preset 传给 Host，因此两类会话不会混排。

QZH 模式是去目录化的 SaaS 界面：浏览器把它的会话渲染为单列表（无工作区分组、无分组标题、无添加工作区或视图选项入口），不受持久化的 `groupBy` 偏好影响——该偏好仍归通用模式所有。
