...
- **Full-log archives** — `uploadEvidenceArchive` stores the original zip under `<evidenceRoot>/<caseId>/archive.zip` and its extracted tree under `<evidenceRoot>/<caseId>/files/` (default evidence root `<DSH_HOME>/qzh-evidence`, overridable via `evidenceRoot`). Extraction validates every entry path, rejects non-file/symlink entries, bounds archive/file/file-count sizes, and publishes atomically via a staging rename; a second upload for the same case is refused. `qzh_list_logs` / `qzh_search_logs` / `qzh_read_log_range` read only through the service (ownership + containment checked) and redact every returned excerpt. Summaries remain the durable fallback when no archive was uploaded.
