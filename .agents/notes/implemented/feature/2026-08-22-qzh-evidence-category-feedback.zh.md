...
跨机分析能从端到端区分服务端与终端证据，两端不会在错误聚类中混并。报告质量通过裁决变得可度量，代价是证据载荷略大、案例多一个字段。反馈目前按案例、暂存内存，与既有的延迟案例持久化一致；尚未接入聚合质量看板或通知事件。

## 相关

基于 `2026-08-22-qzh-evidence-layout.md`（布局规则、逐文件样例、`qzh_list_evidence`）；工作台与详情栏集成见 `2026-08-21-qzh-details-panel.md`。
